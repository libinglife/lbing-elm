<template>
  <div class="shopList_container">
    <ul v-load-more ="loaderMore" v-if="shopListArr.length" type="1">
      <router-link to class="shop_li" v-for="item in shopListArr" tag="li" :key="item.id">
         <section>
        ...
        <div class="shop_right">
            <header class="shop_detail_header">
              ...
              <ul class="shop_detail_ul">
                <li v-for="item_item in item.supports" :key="item_item.id" class="supports">{{item_item.icon_name}}</li>
              </ul>
            </header>
...
<script>
    import {shopList , hotcity} from '../../service/getData'
    import {imgBaseUrl} from 'src/config/env';
    import {mapState} from 'vuex'
    import {showBack} from 'src/config/utils'
    ...
    export default {
      ...
      methods:{
        ...
        zhunshi(supports){
          ...
          //监听父级传过来的参数变化时，触发此函数重新根据属性值获取数据
...
<style lang="scss" scoped>
  ...
  .loading-enter-active , .load-leave-active{
      transition: opacity 1s;
  }
  .loading-enter , .load-leave-active{
      opacity: 0;
  }
